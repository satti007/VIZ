...
class Tree(object):
	...
	def getChildren(self, idx):
		...
		# print("sort", idx,tlist)
		tlist = sorted(tlist)
		# print("Before", idx,tlist)
		if self.lr==0:
			# print ('reverse:', idx)
			tlist.reverse()
			# print("After", idx,tlist)
		return tlist
...
def alphaBeta(tree,node_id,depth,alpha,beta,maxPlay,path):
	...
	if maxPlay:
		v = MIN
		# print ("ChildM",tree.getChildren(node_id))
		...
		v = MAX
		# print ("ChildI",tree.getChildren(node_id))
		for i in tree.getChildren(node_id):
			path.append(i)
...
def Algorithm(adjacency_list):
	tree = Tree(adjacency_list,1)
	path = [len(adjacency_list)-1]
	best = [len(adjacency_list)-1]
	for i in range(0,len(adjacency_list)-1):
		curr_node = tree.globalDict[i+1]
	# 	print ("id ",curr_node.id)
	# 	print ("val ",curr_node.value)
	# 	print ("children ",tree.getChildren(curr_node.id))
	# 	print ("")

	# print ('################')
	path = alphaBeta(tree,len(adjacency_list)-1,maxDepth,MIN,MAX,True,path)[1]
	for i in range(0,len(adjacency_list)-1):
		curr_node = tree.globalDict[i+1]
		# print ("id ",curr_node.id)
		# print ("val ",curr_node.value)
	...
	# 		path[x], path[x+1] = path[x+1], path[x]
	# 		x += 2
	# 	else:
	# 		x += 1
...
def my_main(n_node = 10, bf = 2,gg=1):
	...
	if (gg==1):
		...
		save_ToPickle([tree_nodes,tree_edge_list,adjacency_list])
		
		return tree_nodes,tree_edge_list,adjacency_list
...
class Example(QMainWindow):
	...
	def findClosestCoordinate(self,min_x,min_y,x,y):
		...
		if(self.init_phase==0):#initial phase for planar graph generation
			...
			ret_values = Algorithm(adjacency_list)
...
if __name__ == '__main__':
	...
	ex = Example()
	ex.show()
	# tree_nodes,tree_edge_list,adjacency_list =  my_main(n_node = 10, bf = 2,gg=1)
	# path = Algorithm(adjacency_list)
	sys.exit(app.exec_())